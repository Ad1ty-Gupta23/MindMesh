...
export default function Game() {
  ...
  const [userStats, setUserStats] = useState({ xpPoints: 0 });

  const userId = user?.uid;

  // Initialize user stats and high score
  useEffect(() => {
    const initializeData = async () => {
      // Load local high score
      const saved = JSON.parse(
        sessionStorage.getItem("fruitCatcherData") || '{"highScore": 0}'
      );
      setHighScore(saved.highScore);

      // Load user XP from Firebase
      if (userId) {
        try {
  ...
  const addXP = async (points) => {
    if (!userId) {
      toast.error("Please log in to earn XP rewards!");
      return false;
    }
    ...
    try {
      ...
      if (userDoc.exists()) {
        const userData = userDoc.data();
        const newXP = (userData.xpPoints || 0) + points;
        ...
        setUserStats(prev => ({ ...prev, xpPoints: newXP }));
        return true;
      } else {
        ...
        setUserStats(prev => ({ ...prev, xpPoints: points }));
        return true;
      }
    } catch (error) {
  ...
  const handleClaimReward = async () => {
    const xpReward = Math.floor(score / 100);
    if (xpReward === 0) {
      toast.info("Score too low for reward (need 100+ score)");
      return;
    }

    if (!userId) {
      toast.error("Please log in to claim XP rewards!");
      return;
    }
    ...
    if (success) {
      toast.success(`🎉 You earned ${xpReward} XP! Total XP: ${userStats.xpPoints + xpReward}`);
      ...
      setFruits((prev) => {
        ...
        updated.forEach((fruit) => {
          ...
          if (isCaught) {
            ...
            // Visual effects
            addParticles(
              fruit.x + FRUIT_SIZE / 2,
              365,
              fruit.type.color.replace("text-", ""),
              12
            );
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 p-4">
      ...
      {/* User Stats Display */}
      {userId && (
        <div className="mb-4 bg-black bg-opacity-30 backdrop-blur-sm p-3 rounded-xl text-white text-center">
          <p className="text-lg font-semibold">
            Total XP: <span className="text-yellow-400">{userStats.xpPoints}</span>
          </p>
        </div>
      )}
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div
            className="absolute top-20 right-20 w-16 h-8 bg-white rounded-full opacity-80 animate-pulse"
            style={{ animationDelay: "1s" }}
          ></div>
          <div
            className="absolute top-12 left-1/2 w-24 h-10 bg-white rounded-full opacity-75 animate-pulse"
            style={{ animationDelay: "2s" }}
          ></div>

          {/* Sun */}
          <div
            className="absolute top-4 right-4 w-12 h-12 bg-yellow-400 rounded-full animate-spin"
          ...
          <div className="flex justify-between items-center text-white">
            ...
            <div className="flex flex-col items-end">
              <span className="text-lg font-bold">
                Missed:
                <span
                  className={missed >= 3 ? "text-red-400" : "text-green-400"}
                >
                  {" "}
                  {missed}/5
                </span>
        ...
        {gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-80 backdrop-blur-sm text-white p-8 rounded-xl shadow-2xl border border-purple-500">
            <h2 className="text-5xl font-extrabold mb-6 text-red-500 animate-pulse drop-shadow">
              Game Over!
            </h2>
            ...
            <div className="bg-gradient-to-r from-purple-700 via-indigo-800 to-blue-900 bg-opacity-70 p-6 rounded-2xl mb-6 text-center w-72 shadow-inner border border-white/20">
              ...
              {score === highScore && score > 0 && (
                <p className="text-md text-pink-400 mt-3 font-semibold animate-bounce">
                  🎉 New High Score! 🎉
                </p>
              )}
            </div>

            {!rewardClaimed && sessionComplete && score >= 100 && (
              <div className="mb-4 text-center">
                {!userId ? (
                  <p className="text-yellow-300 mb-2">Login to claim XP rewards!</p>
                ) : (
                  <button
                    onClick={handleClaimReward}
                    className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 px-6 py-2 rounded-xl text-white font-semibold transition-all duration-200 shadow-lg hover:shadow-green-500/20"
                  >
                    Claim {Math.floor(score / 100)} XP
                  </button>
                )}
              </div>
            )}
        ...
        {!gameStarted && !gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm text-white p-6">
            ...
            <div className="bg-white bg-opacity-10 p-5 rounded-xl mb-8 backdrop-blur-sm">
              <div className="grid grid-cols-1 gap-3 text-lg text-black">
                ...
                <p>⭐ Score 100+ points to earn XP!</p>
                {!userId && <p className="text-yellow-300">🔑 Login to earn XP rewards!</p>}
              </div>
            </div>
            <button
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm text-white">
            ...
            <div className="bg-white bg-opacity-10 p-6 rounded-xl text-center">
              ...
              <div className="text-lg opacity-80">
                ...
                <p>Missed: {missed}/5</p>
                {combo > 1 && (
                  <p className="text-yellow-300">Combo: {combo}x</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 text-center text-gray-300 bg-black bg-opacity-30 p-4 rounded-xl backdrop-blur-sm">
        <p className="text-lg">
          Controls: ← → Arrow Keys or A/D | P to Pause | R to Restart
        </p>
        <p className="text-sm opacity-70 mt-2">
          Catch rare fruits for bonus points! Build combos for multipliers!
        </p>
        {!userId && (
          <p className="text-yellow-300 text-sm mt-2">
            Login to your account to earn and save XP rewards!
          </p>
        )}
      </div>
    </div>
